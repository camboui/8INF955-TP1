...
public class Main {
	...
	public static void main(String[] args) {
		...
		walls.add(new GameObject(new AABB(width - 3, 0, width, height), 0, 0));
		walls.add(new GameObject(new AABB(0, height - 3, width, height), 0, 0));
...
//		entities.add(new GameObject(c, 0.1f, 0.5f));
//		
//		OBB obb = new OBB(new Position(20,20), 40, 90, Math.PI/3);
//		entities.add(new GameObject(obb, 0.1f, -0.5f));
//		
//		AABB aabb = new AABB(350, 400, 80, 45);
//		entities.add(new GameObject(aabb, -0.1f, -0.5f));
//		
//		Point point = new Point(70,100);
//		entities.add(new GameObject(point, -0.1f, -0.5f));
	...
	public static void applyLogic(List<GameObject> go, List<GameObject> walls) {
		...
		for (GameObject current : go) {
			current.applyMove();
			if (current.getShape().isCollideGo(walls)) {
